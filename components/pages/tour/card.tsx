...
const TourCard: React.FC<TourCardProps> = ({ tour }) => {
    return (
        <Card className={cn("w-[380px]")}>
            <CardHeader>
                <Image src={`/assets/image/products/${tour.image}/2.jpg`} alt={tour.title} width={380} height={250} className="w-full h-48 object-cover mb-4 rounded-lg"/>
                <CardTitle>
                    ...
                    <span id={'services'}>
                        <ul className="grid grid-cols-1 md:grid-cols-2 gap-3 list-inside my-4">
                            {tour.groupTour ? (
                                ...
                                <>
                                    {tour.privateTour.include.slice(1, 3).map((item, index) => (
                                        <li key={index} className="flex items-center">
                                            <span className="mr-2">{item.icon}</span>
                                            <span>{item.name}</span>
                                        </li>
